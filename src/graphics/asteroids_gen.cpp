...
std::vector<std::pair<glm::vec3, uint32_t>> generateAsteroids(std::mt19937& rng, std::span<const AsteroidVariant> variants) {
	...
	std::uniform_int_distribution<int> variantDist(0, (int)variants.size() - 1);
	...
	auto addAsteroid = [&] (const glm::vec3& pos, uint32_t variant) -> bool {
		...
		float thisVarRadius = variants[variant].size;
		bool ok = iterateAround(pos, thisVarRadius + CELL_SIZE, [&] (int x, int y, int z) {
			if (cellData->data[x][y][z] != -1) {
				...
				float maxRad = variants[otherVariant].size + thisVarRadius + SPACING;
		...
		activeSet.push_back({ pos, variants[variant].size, variant, MAX_ATTEMPTS });
	...
	float firstRadius = variants[firstVariant].size;
	...
	while (!activeSet.empty()) {
		...
		glm::vec3 pos = activeSet[idx].pos + randomDirection(rng) * (activeSet[idx].radius + SPACING + variants[variant].size);
